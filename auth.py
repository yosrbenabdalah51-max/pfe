...
ROLE_PAGES = {
    ...
    "manager": ["App", "Comparaison Modèles", "SARIMA", "LSTM", "XGBoost", "Stock Management"],
    "stock":   ["App", "Stock Management"],
}

def login_page():
    # CSS pour centrer et réduire la largeur du formulaire
    st.markdown("""
    <style>
...
def user_topbar():
    """
    Affiche le nom de l'utilisateur connecté + bouton déconnexion
    EN DEHORS de la sidebar, en haut à droite de la page principale.
    ...
    .topbar-role {{
        font-size: 11px; color: #9ca3af; font-weight: 500; text-transform: uppercase; letter-spacing: 1px;
    }}
    </style>
...
def require_auth(page_name: str):
    ...
    if page_name not in allowed_pages:
        st.error(f"⛔ Accès refusé. Votre rôle **{role}** n'a pas accès à cette page.")
        st.stop()